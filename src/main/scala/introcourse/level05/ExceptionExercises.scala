...
object ExceptionExercises {
  ...
  def createPerson(name: String, age: String): Person = {
    //    what does the signature of the function tell us? Talk about local reasoning
  ...
  def createValidPeople: List[Person] = {
    ...
    //    The return type is List[Person], how do we collect that?
    list.collect {
      case p: Person => p //must specify the type
    }
//    Why do we collect?
  }
